...
class ExprTools {
	...
	static public function map(source:Expr, f:Expr->Array<VarDecl>->Expr, ctx:Array<VarDecl>, ?pos:Position):Expr {
		...
		var ret = switch(mappedSource.expr) {
			case ECheckType(e, t): trace("--- ECheckType ---"); ECheckType(rec(e), t);
			case ECast(e, t): trace("--- ECast ---");ECast(rec(e), t);
			case EArray(e1, e2): trace("--- EArray ---");EArray(rec(e1), rec(e2));
			case EField(e, field): trace("--- EField ---");EField(rec(e), field);
			case EParenthesis(e):  trace("--- EParenthesis ---");EParenthesis(rec(e));
			case ECall(e, params): trace("--- ECall ---");ECall(rec(e), mapArray(params, f, ctx, pos));
			case EIf(econd, eif, eelse): trace("--- EIf ---");EIf(rec(econd), rec(eif), rec(eelse));
			case ETernary(econd, eif, eelse): trace("--- ETernary ---");ETernary(rec(econd), rec(eif), rec(eelse));
			case EBlock(exprs): trace("--- EBlock ---");EBlock(exprs.mapArray(f, ctx.copy(), pos));
			case EArrayDecl(exprs): trace("--- EArrayDecl ---");EArrayDecl(exprs.mapArray(f, ctx, pos));
			case EIn(e1, e2): trace("--- EIn ---");EIn(rec(e1), rec(e2));
			case EWhile(econd, e, normalWhile): trace("--- EWhile ---");EWhile(rec(econd), rec(e), normalWhile);
			case EUntyped(e): trace("--- EUntyped ---");EUntyped(rec(e));
			case EThrow(e): trace("--- EThrow ---");EThrow(rec(e));
			case EReturn(e): trace("--- EReturn ---");EReturn(rec(e));
			case EDisplay(e, t): trace("--- EDisplay ---");EDisplay(rec(e), t);
			case EDisplayNew(t): trace("--- EDisplayNew ---");EDisplayNew(t);
			case EUnop(op, postFix, e): trace("--- EUnop ---");EUnop(op, postFix, rec(e));
			case ENew(t, params): trace("--- ENew ---");ENew(t, params.mapArray(f, ctx, pos));
			case EBinop(op, e1, e2): trace("--- EBinop ---");EBinop(op, rec(e1), rec(e2));
			case EObjectDecl(fields):trace("--- EObjectDecl ---");
				var newFields = [];
				for (field in fields)
					newFields.push( { field:field.field, expr:rec(field.expr) } );
				EObjectDecl(newFields);
			case ESwitch(expr, cases, def):trace("--- ESwitch ---");
				var newCases = cases;
				newCases = [];
				expr = rec(expr);
			...
			case ETry(e, catches):trace("--- ETry ---");
				var newCatches = [];
				for (c in catches)
				{
					var innerCtx = ctx.copy();
			...
			case EFunction(name, func):trace("--- EFunction ---");
				var innerCtx = ctx.copy();
